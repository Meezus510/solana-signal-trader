...
Recommended policy:
  - run after 25 new closed trades, or
  - every 6 hours if at least 10 new closed trades accumulated
This avoids overfitting to tiny samples while still reacting during regime shifts.
"""
...
def _last_run_meta() -> tuple[int, datetime | None]:
    cfg = load_config()
    meta = cfg.get("_meta", {})
    count = int(meta.get("openai_manager_last_closed_count", 0))
    ts_raw = meta.get("openai_manager_last_run_at")
    if not ts_raw:
        return count, None
...
def _store_meta(current_closed: int) -> None:
    ...
    cfg["_meta"]["openai_manager_last_closed_count"] = current_closed
    cfg["_meta"]["openai_manager_last_run_at"] = datetime.now(timezone.utc).isoformat()
    save_owned_config(
        original,
        cfg,
